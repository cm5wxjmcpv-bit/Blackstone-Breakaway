...
export class IncrementalGame {
  ...
  reconcileState(snapshot) {
    ...
    this.config.generators.forEach((generator) => {
      ...
      const assignedMineId = state.generatorAssignments[generator.id];
      const assignedMine = this.config.minesById[assignedMineId];
      if (!assignedMine || !state.unlockedMines.includes(assignedMineId)) {
        state.generatorAssignments[generator.id] = state.unlockedMines.includes(state.currentMine)
          ? state.currentMine
          : this.config.start.mineId;
        changed = true;
      }
      const mineId = state.generatorAssignments[generator.id];
      const mine = this.config.minesById[mineId];
      const savedDeposit = state.automationDeposits[generator.id];
      const deposit = savedDeposit ? this.config.depositsById[savedDeposit.id] : null;
      if (!deposit
        || !mine.depositIds.includes(deposit.id)
        || savedDeposit.maxHp !== deposit.maxHp
        || savedDeposit.hp > deposit.maxHp) {
        const replacement = this.config.depositsById[mine.depositIds[0]];
        state.automationDeposits[generator.id] = {
          id: replacement.id,
  ...
  isStateCompatible(snapshot) {
    ...
    if (!Object.entries(snapshot.generatorAssignments).every(([generatorId, mineId]) => (
      knownGenerators.has(generatorId)
      && Boolean(this.config.minesById[mineId])
      && snapshot.unlockedMines.includes(mineId)
    ))) return false;
    if (!Object.entries(snapshot.automationDeposits).every(([generatorId, savedDeposit]) => {
      if (!knownGenerators.has(generatorId)) return false;
      const mineId = snapshot.generatorAssignments[generatorId];
      const mine = this.config.minesById[mineId];
      const deposit = this.config.depositsById[savedDeposit.id];
      return Boolean(mine) && (!deposit || savedDeposit.hp <= savedDeposit.maxHp);
    })) return false;
  ...
  getSkillUnlockStatus(skillId) {
    const id = normalizedId(skillId);
    const skill = this.config.skillsById[id];
    if (!skill) return { unlocked: false, reason: 'unknown-skill', skillId: id, prerequisites: [] };
    if (!skill.enabled) {
      return { unlocked: false, reason: 'skill-disabled', skillId: id, prerequisites: [] };
    }
    const prerequisites = skill.prerequisites.map((requirement) => {
      const requiredSkill = this.config.skillsById[requirement.skillId];
      const currentRank = this.getSkillRank(requirement.skillId);
      return {
        ...requirement,
        name: requiredSkill?.name || requirement.skillId,
        currentRank,
        met: currentRank >= requirement.rank,
      };
    });
    const unmet = prerequisites.find((requirement) => !requirement.met) || null;
    return {
      unlocked: !unmet,
      reason: unmet ? 'prerequisite' : null,
      skillId: id,
      prerequisites,
      unmet,
    };
  ...
  getMiningStats() {
    const baseManualPower = this.config.balance.manualPower + this.getMiningBonus('manual-power-flat');
    const miningSpeed = 1 + this.getMiningBonus('mining-speed');
    return {
      manualPower: safeMultiply(baseManualPower, miningSpeed),
      miningSpeed,
      ...
      oreYieldMultiplier: 1 + this.getMiningBonus('ore-yield-multiplier'),
      rareFindChance: this.getRareFindChance(),
      xpMultiplier: 1 + this.getMiningBonus('xp-bonus'),
  ...
  getGeneratorAssignment(generatorId) {
    const id = normalizedId(generatorId);
    return this.state?.generatorAssignments?.[id] || this.config.start.mineId;
  }

  getAutomationDeposit(generatorId) {
    const id = normalizedId(generatorId);
    const saved = this.state?.automationDeposits?.[id];
    return saved ? clone(saved) : null;
  }

  assignGeneratorToMine(generatorId, mineId) {
    if (!this.state) throw new Error('IncrementalGame must be started before assigning automation.');
    const id = normalizedId(generatorId);
    const targetMineId = normalizedId(mineId);
    const generator = this.config.generatorsById[id];
    const mine = this.config.minesById[targetMineId];
    if (!generator) return { ok: false, reason: 'unknown-generator', generatorId: id };
    if (!mine) return { ok: false, reason: 'unknown-mine', mineId: targetMineId };
    if (this.getGeneratorOwned(id) < 1) {
      return { ok: false, reason: 'generator-not-owned', generatorId: id };
    }
    if (!this.state.unlockedMines.includes(targetMineId)) {
      return { ok: false, reason: 'locked-mine', mineId: targetMineId };
    }
    if (this.getGeneratorAssignment(id) === targetMineId) {
      return { ok: true, reason: 'already-assigned', generatorId: id, mineId: targetMineId };
    }
    const deposit = this.selectNextDeposit(targetMineId, { applyMiningEvent: false });
    this.state.generatorAssignments[id] = targetMineId;
    this.state.automationDeposits[id] = {
      id: deposit.id,
      hp: deposit.maxHp,
      maxHp: deposit.maxHp,
    };
    ...
    this.saveCheckpoint('automation-assignment');
    this.emit('automation-assignment', result);
    return result;
  }
  ...
  purchaseGenerator(generatorId) {
    ...
    const previouslyOwned = this.state.generators[id];
    this.state.generators[id] += 1;
    if (previouslyOwned === 0 && this.state.unlockedMines.includes(this.state.currentMine)) {
      const deposit = this.selectNextDeposit(this.state.currentMine, { applyMiningEvent: false });
      this.state.generatorAssignments[id] = this.state.currentMine;
      this.state.automationDeposits[id] = {
        id: deposit.id,
        hp: deposit.maxHp,
        maxHp: deposit.maxHp,
  ...
  getAutomationStats() {
    ...
    const generators = this.config.generators.map((generator) => {
      ...
      return {
        id: generator.id,
        owned,
        mineId: this.getGeneratorAssignment(generator.id),
        deposit: this.getAutomationDeposit(generator.id),
        basePower,
        generatorBonus,
        power,
        effectivePower: safeMultiply(safeMultiply(power, globalMultiplier), acquisitionMultiplier),
      };
    });
    ...
    return {
      ...
      xpPerSecond: this.getAutomationXpPerSecond(),
      generators,
    };
  }
  ...
  getAutomationXpPerSecond() {
    if (!this.state?.company?.created) return 0;
    const workers = this.config.generators.reduce((total, generator) => safeAdd(
      total,
      safeMultiply(this.getGeneratorOwned(generator.id), generator.workersPerUnit),
    ), 0);
    const workerXp = safeMultiply(workers, this.config.balance.workerXpPerSecond);
    const skillXp = this.getSkillBonus('automation-xp-per-second');
    const trainingXp = this.config.businessUpgrades.reduce((total, upgrade) => (
      upgrade.effect.type === 'automation-xp-per-second'
        ? safeAdd(total, safeMultiply(this.getBusinessUpgradeRank(upgrade.id), upgrade.effect.amount))
        : total
    ), 0);
    return safeAdd(workerXp, safeAdd(skillXp, trainingXp));
  }
  ...
  allocateSkill(skillId) {
    ...
    const unlock = this.getSkillUnlockStatus(id);
    if (!unlock.unlocked) {
      return {
        ok: false,
        reason: unlock.reason === 'prerequisite' ? 'missing-prerequisite' : 'skill-locked',
        skillId: id,
  ...
  purchaseEquipment(itemId, options = {}) {
    ...
    if (this.state.character.level < item.requiredLevel) {
      return {
        ok: false,
        reason: 'level-required',
        itemId: id,
  ...
  buyScratchTicket(ticketId) {
    ...
    if (this.state.character.level < ticket.requiredLevel) {
      return {
        ok: false,
        reason: 'level-required',
        ticketId: id,
        requiredLevel: ticket.requiredLevel,
        level: this.state.character.level,
      };
    }
  ...
  breakDeposit(deposit, damage, hit = {}) {
    ...
    const rewardMultiplier = safeMultiply(
      safeMultiply(mineRewardMultiplier, skillRewardMultiplier),
  ...
  breakAutomationDeposit(generatorId, mineId, deposit) {
    const resource = this.config.resourcesById[deposit.resourceId];
    const baseQuantity = rollDepositReward(deposit, this.random);
    const mineRewardMultiplier = this.config.minesById[mineId]?.rewardMultiplier || 1;
    const quantity = Math.max(
      baseQuantity,
      Math.floor(safeMultiply(baseQuantity, mineRewardMultiplier)),
    );
    this.state.materials[resource.id] = safeAdd(this.state.materials[resource.id], quantity);
    this.state.statistics.totalDepositsBroken = safeAdd(this.state.statistics.totalDepositsBroken, 1);
    this.state.statistics.totalOreMined = safeAdd(this.state.statistics.totalOreMined, quantity);
    ...
    const mineProgress = this.getMineProgress(mineId);
    mineProgress.depositsBroken = safeAdd(mineProgress.depositsBroken, 1);
    mineProgress.oreMined = safeAdd(mineProgress.oreMined, quantity);
    const nextDeposit = selectWeightedDeposit(this.config, mineId, this.random);
    this.state.automationDeposits[generatorId] = {
      id: nextDeposit.id,
      hp: nextDeposit.maxHp,
      maxHp: nextDeposit.maxHp,
    };
    return {
      ...
      baseQuantity,
      mineRewardMultiplier,
      grossValue: safeMultiply(quantity, resource.value),
      nextDepositId: nextDeposit.id,
      mineProgress: clone(mineProgress),
    };
  }
  ...
  applyAutomation(deltaSeconds, options = {}) {
    ...
    if (!this.state?.company?.created) return emptyResult;

    const automation = this.getAutomationStats();
    const productionPower = automation.totalPower;
    const productionMultiplier = Number.isFinite(options.productionMultiplier)
      ? Math.max(0, options.productionMultiplier)
      : 1;
    ...
    const activeSites = automation.generators.filter((entry) => entry.owned > 0 && entry.effectivePower > 0);
    const baseSiteBreakLimit = Math.floor(maxBreaks / Math.max(1, activeSites.length));
    const extraBreakSlots = maxBreaks % Math.max(1, activeSites.length);
    let damage = 0;
    let depositsBroken = 0;
    let remainingDamageTotal = 0;
    let limited = false;
    const resources = {};
    const breaks = [];
    const sites = [];

    activeSites.forEach((entry, siteIndex) => {
      const siteBreakLimit = baseSiteBreakLimit + (siteIndex < extraBreakSlots ? 1 : 0);
      const mineId = this.getGeneratorAssignment(entry.id);
      let remainingDamage = safeMultiply(
        safeMultiply(entry.effectivePower, deltaSeconds),
        productionMultiplier,
      );
      let siteDamage = 0;
      let siteBreaks = 0;
      const siteResources = {};
      const siteBreakResults = [];
      while (remainingDamage > 0 && siteBreaks < siteBreakLimit) {
        const savedDeposit = this.state.automationDeposits[entry.id];
        const deposit = savedDeposit ? this.config.depositsById[savedDeposit.id] : null;
        if (!deposit) break;
        const appliedDamage = Math.min(remainingDamage, savedDeposit.hp);
        savedDeposit.hp = Math.max(0, savedDeposit.hp - appliedDamage);
        siteDamage = safeAdd(siteDamage, appliedDamage);
        remainingDamage = Math.max(0, remainingDamage - appliedDamage);
        if (savedDeposit.hp > 0) break;

        const breakResult = this.breakAutomationDeposit(entry.id, mineId, deposit);
        siteBreaks += 1;
        siteResources[breakResult.resourceId] = safeAdd(
          siteResources[breakResult.resourceId] || 0,
          breakResult.quantity,
        );
        resources[breakResult.resourceId] = safeAdd(
          resources[breakResult.resourceId] || 0,
          breakResult.quantity,
        );
        if (collectBreaks) {
          breaks.push(breakResult);
          siteBreakResults.push(breakResult);
        }
      }
      const siteLimited = remainingDamage > 0 && siteBreaks >= siteBreakLimit;
      damage = safeAdd(damage, siteDamage);
      depositsBroken = safeAdd(depositsBroken, siteBreaks);
      remainingDamageTotal = safeAdd(remainingDamageTotal, remainingDamage);
      limited ||= siteLimited;
      sites.push({
        generatorId: entry.id,
        mineId,
        productionPower: entry.effectivePower,
        damage: siteDamage,
        depositsBroken: siteBreaks,
        resources: siteResources,
        breaks: siteBreakResults,
        limited: siteLimited,
        remainingDamage,
        deposit: this.getAutomationDeposit(entry.id),
      });
    });

    const xpPerSecond = automation.xpPerSecond;
    const xp = safeMultiply(xpPerSecond, deltaSeconds);
    let levelResult = {
      levelsGained: 0,
      skillPointsGained: 0,
      level: this.state.character.level,
    ...
    if (xp > 0) {
      ...
      this.state.statistics.totalAutomatedXp = safeAdd(
        this.state.statistics.totalAutomatedXp,
        xp,
      );
      levelResult = this.applyLevelProgression();
      if (levelResult.levelsGained > 0) this.emit('level-up', levelResult);
    }
    const milestones = depositsBroken > 0 ? this.evaluateMilestones() : [];
    const result = {
      damage,
      depositsBroken,
      resources,
      ...
      sites,
      limited,
      remainingDamage: remainingDamageTotal,
      milestones,
    };
    if ((depositsBroken > 0 || xp > 0) && options.emit !== false) this.emit('automation', result);
    return result;
  }
    ...
    if (window.reason === 'future-timestamp') {
      ...
      if (window.eligible) {
        ...
        Object.assign(result, {
          ...
          xpPerSecond: automation.xpPerSecond,
          xp: automation.xp,
          levelsGained: automation.levelsGained,
