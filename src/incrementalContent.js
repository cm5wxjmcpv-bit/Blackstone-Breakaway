...
function normalizeSkillBranches(rawBranches, errors) {
  if (rawBranches !== undefined && !Array.isArray(rawBranches)) {
    errors.push('skillBranches must be an array when provided');
  }
  const source = Array.isArray(rawBranches) && rawBranches.length
    ? rawBranches
    : [{ id: 'general', name: 'General Skills', description: 'Core progression skills.' }];
  const branches = source.map((entry) => ({
    id: normalizedId(entry?.id),
    name: text(entry?.name, entry?.id || 'Skill Branch', 80),
    description: text(entry?.description, '', 200),
  }));
  uniqueIds(branches, 'skillBranches', errors);
  return branches;
}

function normalizeSkills(rawSkills, skillBranches, errors) {
  if (rawSkills !== undefined && !Array.isArray(rawSkills)) {
    errors.push('skills must be an array when provided');
  }
  const branchIds = new Set(skillBranches.map((branch) => branch.id));
  const defaultBranchId = skillBranches[0]?.id || 'general';
  const skills = (Array.isArray(rawSkills) ? rawSkills : []).map((entry, index) => {
    const effectType = normalizedId(entry?.effect?.type);
    const branchId = normalizedId(entry?.branchId) || defaultBranchId;
    const tier = integer(entry?.tier, 1, 1);
    if (!Number.isInteger(entry?.maxRank) || entry.maxRank < 1) {
      errors.push(`skills[${index}].maxRank must be a positive integer`);
    }
    if (entry?.tier !== undefined && (!Number.isInteger(entry.tier) || entry.tier < 1)) {
      errors.push(`skills[${index}].tier must be a positive integer`);
    }
    if (!branchIds.has(branchId)) {
      errors.push(`skills[${index}].branchId references missing branch "${branchId || '(invalid)'}"`);
    }
    if (entry?.prerequisites !== undefined && !Array.isArray(entry.prerequisites)) {
      errors.push(`skills[${index}].prerequisites must be an array when provided`);
    }
    ...
    return {
      ...
      branchId,
      tier,
      prerequisites: (Array.isArray(entry?.prerequisites) ? entry.prerequisites : []).map(
        (requirement, requirementIndex) => {
          if (!Number.isInteger(requirement?.rank) || requirement.rank < 1) {
            errors.push(`skills[${index}].prerequisites[${requirementIndex}].rank must be a positive integer`);
          }
          return {
            skillId: normalizedId(requirement?.skillId),
            rank: integer(requirement?.rank, 1, 1),
          };
        },
      ),
  ...
  const skillIds = uniqueIds(skills, 'skills', errors);
  const skillsById = Object.fromEntries(skills.map((skill) => [skill.id, skill]));
  skills.forEach((skill) => {
    const seenRequirements = new Set();
    skill.prerequisites.forEach((requirement) => {
      const requiredSkill = skillsById[requirement.skillId];
      if (!skillIds.has(requirement.skillId)) {
        errors.push(`skill "${skill.id || '(invalid)'}" requires missing skill "${requirement.skillId || '(invalid)'}"`);
      } else if (requirement.skillId === skill.id) {
        errors.push(`skill "${skill.id || '(invalid)'}" cannot require itself`);
      } else if (seenRequirements.has(requirement.skillId)) {
        errors.push(`skill "${skill.id || '(invalid)'}" contains duplicate prerequisite "${requirement.skillId}"`);
      } else if (requirement.rank > requiredSkill.maxRank) {
        errors.push(`skill "${skill.id || '(invalid)'}" requires an unattainable rank of "${requirement.skillId}"`);
      } else if (requiredSkill.branchId === skill.branchId && requiredSkill.tier >= skill.tier) {
        errors.push(`skill "${skill.id || '(invalid)'}" must require an earlier tier in its branch`);
      }
      seenRequirements.add(requirement.skillId);
    });
  });
  ...
  function visitSkill(skill) {
    ...
    if (visiting.has(skill.id)) {
      errors.push(`skill "${skill.id || '(invalid)'}" has a circular prerequisite chain`);
      return;
    }
    visiting.add(skill.id);
    skill.prerequisites.forEach((requirement) => visitSkill(skillsById[requirement.skillId]));
    visiting.delete(skill.id);
    visited.add(skill.id);
  }
  skills.forEach(visitSkill);
  return skills;
}
...
function normalizeEquipment(rawEquipment, errors) {
  ...
  const items = (Array.isArray(source.items) ? source.items : []).map((entry, itemIndex) => {
    ...
    if (entry?.requiredLevel !== undefined
      && (!Number.isInteger(entry.requiredLevel) || entry.requiredLevel < 1)) {
      errors.push(`equipment.items[${itemIndex}].requiredLevel must be a positive integer`);
    }
    ...
    return {
      ...
      requiredLevel: integer(entry?.requiredLevel, 1, 1),
...
function normalizeLottery(rawLottery, resourcesById, errors) {
  ...
  const tickets = (Array.isArray(source.scratchTickets) ? source.scratchTickets : []).map((entry, ticketIndex) => {
    ...
    if (entry?.requiredLevel !== undefined
      && (!Number.isInteger(entry.requiredLevel) || entry.requiredLevel < 1)) {
      errors.push(`lottery.scratchTickets[${ticketIndex}].requiredLevel must be a positive integer`);
    }
...
function normalizeGenerators(rawGenerators, company, errors) {
  ...
  const generators = (Array.isArray(rawGenerators) ? rawGenerators : []).map((entry, index) => {
    ...
    const visualType = normalizedId(entry?.visualType) || 'miner';
    ...
    if (entry?.visualType !== undefined && !GENERATOR_VISUAL_TYPES.has(visualType)) {
      errors.push(`${label}.visualType must be miner, crew, or drill`);
    }
    return {
      id: normalizedId(entry?.id),
      ...
      visualType: GENERATOR_VISUAL_TYPES.has(visualType) ? visualType : 'miner',
...
function normalizeBusinessUpgrades(rawUpgrades, company, generatorsById, errors) {
  ...
  const upgrades = (Array.isArray(rawUpgrades) ? rawUpgrades : []).map((entry, index) => {
    ...
    if (effectType !== 'generator-multiplier' && entry?.effect?.generatorId !== undefined) {
...
export function normalizeIncrementalConfig(raw, options = {}) {
  ...
  const mines = rawMines.map((entry, index) => {
    if (entry?.rewardMultiplier !== undefined
      && (!isFiniteNumber(entry.rewardMultiplier) || entry.rewardMultiplier < 1)) {
      errors.push(`mines[${index}].rewardMultiplier must be a finite number of at least 1`);
    }
    return {
      id: normalizedId(entry?.id),
  ...
  const workerXpPerSecond = finite(raw.balance?.workerXpPerSecond, 0);
  ...
  if (raw.balance?.workerXpPerSecond !== undefined
    && (!isFiniteNumber(raw.balance.workerXpPerSecond) || raw.balance.workerXpPerSecond < 0)) {
    errors.push('balance.workerXpPerSecond must be a finite nonnegative number');
  }
  ...
  const skillBranches = normalizeSkillBranches(raw.skillBranches, errors);
  ...
  return {
    ...
    balance: {
      ...
      baseCriticalDamage,
      baseOreYieldChance,
      workerXpPerSecond: nonnegative(workerXpPerSecond),
    ...
    story: { milestones },
    skillBranches,
    skillBranchesById: Object.fromEntries(skillBranches.map((entry) => [entry.id, entry])),
    skills,
    equipment,
    lottery,
    store,
