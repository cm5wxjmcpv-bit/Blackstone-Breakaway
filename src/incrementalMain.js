      ...
      <section id="incremental-mine-view" class="incremental-view" role="tabpanel" aria-labelledby="incremental-tab-mine">
        ...
        <section id="incremental-crew-operations" class="incremental-crew-operations incremental-panel" hidden>
          <div class="incremental-crew-heading">
            <div>
              <span class="incremental-label">LIVE COMPANY WORKSITES</span>
              <h2>Your Crews Mine Separately</h2>
              <p>Your personal deposit remains yours. Assign each automation type to an unlocked mine and watch representative workers operate their own ore face.</p>
            </div>
            <div class="incremental-crew-summary">
              <span>Company output</span>
              <strong id="incremental-crew-power">0/sec</strong>
              <small id="incremental-crew-xp">0 XP/sec</small>
            </div>
          </div>
          <p id="incremental-crew-status" class="incremental-section-status" role="status">Purchase automation from the Company page to open a worksite.</p>
          <div id="incremental-crew-grid" class="incremental-crew-grid"></div>
        </section>
      </section>
      ...
      <section id="incremental-company-view" class="incremental-view incremental-section-view" role="tabpanel" aria-labelledby="incremental-tab-company" hidden>
        ...
        <section id="incremental-company-setup" class="incremental-company-setup incremental-panel">
          <div>
            ...
            <p>Registration unlocks separate crew worksites and business upgrades. Workers contribute a small amount of XP, and training programs can increase it.</p>
          </div>
        ...
        <div id="incremental-company-dashboard" class="incremental-company-dashboard" hidden>
          ...
          <section class="incremental-company-section">
            <div class="incremental-category-heading">
              <h2>Automation</h2>
              <p>Each automation type works a separately assigned mine and deposit. Your personal mining target is never replaced by crew production.</p>
            </div>
...
function buildUi(root, database) {
  ...
  const nodes = {
    ...
    crew_operations: byId('incremental-crew-operations'),
    crew_power: byId('incremental-crew-power'),
  ...
  function renderResources(state, employeeStage) {
    ...
    if (resourceRows.size === 0) config.resources.forEach((resource) => {
      ...
      const buttons = [1, 10, 'all'].map((amount) => {
      ...
      resourceRows.set(resource.id, { row, details, count, actions, buttons });
    });

    config.resources.forEach((resource) => {
      const quantity = source[resource.id];
      const refs = resourceRows.get(resource.id);
      refs.row.hidden = !currentResourceIds.has(resource.id) && !(quantity > 0);
      refs.row.classList.toggle('has-sale-actions', !employeeStage);
      refs.details.textContent = employeeStage
      ...
      refs.count.textContent = formatNumber(quantity);
      refs.actions.hidden = employeeStage;
      refs.buttons.forEach(({ amount, button }) => {
        button.disabled = employeeStage || quantity < (amount === 'all' ? 1 : amount);
      });
    });
  }
  ...
  function createCrewVisual(generator) {
    const visual = document.createElement('div');
    visual.className = `incremental-crew-visual is-${generator.visualType}`;
    const actors = document.createElement('div');
    actors.className = 'incremental-crew-actors';
    const representativeCount = generator.visualType === 'crew' ? 3 : 1;
    for (let index = 0; index < representativeCount; index += 1) {
      const actor = document.createElement('span');
      actor.className = generator.visualType === 'drill'
        ? 'incremental-crew-drill'
        : 'incremental-crew-worker';
      actor.style.setProperty('--crew-index', String(index));
      actor.innerHTML = generator.visualType === 'drill'
        ? '<span class="incremental-drill-body"></span><span class="incremental-drill-bit"></span>'
        : '<span class="incremental-worker-head"></span><span class="incremental-worker-body"></span><span class="incremental-worker-tool"></span>';
      actors.appendChild(actor);
    }
    const rock = document.createElement('span');
    rock.className = 'incremental-crew-rock';
    const icon = document.createElement('span');
    icon.className = 'incremental-crew-rock-icon';
  ...
  function ensureCrewCards() {
    if (crewCards.size > 0) return;
    config.generators.forEach((generator) => {
      const card = document.createElement('article');
      card.className = 'incremental-crew-card';
      ...
      const visualRefs = createCrewVisual(generator);
      const depositLine = document.createElement('div');
      depositLine.className = 'incremental-crew-deposit-line';
      const depositName = document.createElement('strong');
      const hp = document.createElement('span');
      depositLine.append(depositName, hp);
      const progress = document.createElement('div');
      progress.className = 'incremental-crew-progress';
      progress.setAttribute('role', 'progressbar');
      const bar = document.createElement('span');
      progress.appendChild(bar);
      const assignmentLabel = document.createElement('label');
      assignmentLabel.textContent = 'Assigned mine';
      const select = document.createElement('select');
      select.dataset.assignGeneratorId = generator.id;
      assignmentLabel.appendChild(select);
      const stats = document.createElement('div');
      stats.className = 'incremental-crew-card-stats';
      const production = document.createElement('strong');
      const reward = document.createElement('small');
      stats.append(production, reward);
      card.append(header, visualRefs.visual, depositLine, progress, assignmentLabel, stats);
      nodes.crew_grid.appendChild(card);
      crewCards.set(generator.id, {
        card,
        count,
        badge,
        select,
  ...
  function syncCrewMineOptions(select, state, selectedMineId) {
    const signature = state.unlockedMines.join('|');
    if (select.dataset.mineSignature !== signature) {
      const options = state.unlockedMines.map((mineId) => {
        const option = document.createElement('option');
        option.value = mineId;
        option.textContent = mineDisplayName(state, config.minesById[mineId]);
        return option;
      });
      select.replaceChildren(...options);
      select.dataset.mineSignature = signature;
    }
    select.value = selectedMineId;
  }

  function renderCrewOperations(game) {
    const state = game.state;
    const automation = game.getAutomationStats();
    ensureCrewCards();
    nodes.crew_operations.hidden = !state.company.created;
    nodes.crew_power.textContent = `${formatNumber(automation.totalPower)}/sec`;
    nodes.crew_xp.textContent = `${formatNumber(automation.xpPerSecond)} XP/sec`;
    config.generators.forEach((generator) => {
      const refs = crewCards.get(generator.id);
      const stats = automation.generators.find((entry) => entry.id === generator.id);
      const owned = stats?.owned || 0;
      const active = owned > 0;
      const mineId = stats?.mineId || config.start.mineId;
      const mine = config.minesById[mineId];
      const savedDeposit = state.automationDeposits[generator.id];
      const deposit = savedDeposit ? config.depositsById[savedDeposit.id] : null;
      const hpProgress = savedDeposit?.maxHp > 0
        ? Math.max(0, Math.min(1, savedDeposit.hp / savedDeposit.maxHp))
        : 0;
      refs.card.classList.toggle('is-operating', active);
      refs.card.classList.toggle('is-unowned', !active);
      refs.badge.textContent = active ? 'OPERATING' : 'NOT OWNED';
      refs.count.textContent = generator.visualType === 'crew'
        ? `${formatNumber(owned)} crew${owned === 1 ? '' : 's'} · ${formatNumber(owned * generator.workersPerUnit)} workers`
        : `${formatNumber(owned)} ${generator.visualType === 'drill' ? 'drill' : 'miner'}${owned === 1 ? '' : 's'}`;
      syncCrewMineOptions(refs.select, state, mineId);
      refs.select.disabled = !active;
      refs.visual.style.setProperty('--crew-mine-background', mine?.visual.background || '#29221f');
      refs.visual.style.setProperty('--crew-mine-accent', mine?.visual.accent || '#f0b94d');
      refs.rock.style.setProperty('--crew-deposit-color', deposit?.visual.color || '#64748b');
      refs.rock.style.setProperty('--crew-deposit-accent', deposit?.visual.accent || '#cbd5e1');
      refs.icon.textContent = deposit?.visual.icon || '';
      refs.depositName.textContent = active
        ? `${deposit?.name || 'Deposit'} · ${mineDisplayName(state, mine)}`
        : 'Purchase this automation type to open its worksite';
      refs.hp.textContent = active && savedDeposit
        ? `${formatNumber(savedDeposit.hp)} / ${formatNumber(savedDeposit.maxHp)} HP`
        : 'Idle';
      refs.bar.style.width = `${hpProgress * 100}%`;
      refs.progress.setAttribute('aria-valuenow', String(savedDeposit?.hp || 0));
      refs.progress.setAttribute('aria-valuemax', String(savedDeposit?.maxHp || 1));
      refs.production.textContent = `${formatNumber(stats?.effectivePower || 0)} mining power/sec`;
      const latest = automationSummaries.get(generator.id);
      refs.reward.textContent = latest
        ? latest
        : active
          ? `Working independently at ${mine?.name || 'assigned mine'}`
          : 'Visible here after purchase';
    });
  }
  ...
  function makeMineCard(game, mine) {
    ...
    yieldChip.className = 'incremental-mine-yield-chip';
    yieldChip.textContent = `${formatNumber(mine.rewardMultiplier, { decimals: 2 })}x ore yield`;
    resources.appendChild(yieldChip);
  ...
  function makeEquipmentCard(game, item, context = 'store') {
    ...
    const levelMet = game.state.character.level >= item.requiredLevel;
    ...
    card.classList.toggle('is-locked', !owned && !levelMet);
    ...
    const levelRequirement = document.createElement('small');
    levelRequirement.className = 'incremental-equipment-requirement';
    levelRequirement.textContent = owned
      ? `Level ${formatNumber(item.requiredLevel)} equipment · Owned`
      : levelMet
        ? `Level ${formatNumber(item.requiredLevel)} requirement met`
        : `Unlocks at character level ${formatNumber(item.requiredLevel)}`;
    card.appendChild(levelRequirement);
    ...
    if (context === 'store') {
      ...
      button.textContent = equipped
        ...
        : owned
          ? 'Owned'
          : !levelMet
            ? `Level ${formatNumber(item.requiredLevel)} Required`
          : !prerequisiteMet
            ? `Requires ${prerequisite.name}`
            : `Buy & Equip · ${formatCurrency(item.cost)}`;
      button.disabled = owned || !levelMet || !prerequisiteMet || game.state.cash < item.cost;
    } else {
  ...
  function makeLotteryCard(game, ticket) {
    ...
    const levelMet = game.state.character.level >= ticket.requiredLevel;
    const card = document.createElement('article');
    card.className = 'incremental-lottery-card';
    card.classList.toggle('is-locked', !levelMet);
    const header = document.createElement('div');
    ...
    unlock.className = 'incremental-lottery-level';
    unlock.textContent = levelMet
      ? `Level ${formatNumber(ticket.requiredLevel)} ticket unlocked`
      : `Unlocks at character level ${formatNumber(ticket.requiredLevel)}`;
    ...
    buy.textContent = levelMet
      ? `Buy Ticket · ${formatCurrency(ticket.cost)}`
      : `Level ${formatNumber(ticket.requiredLevel)} Required`;
    buy.disabled = !levelMet || game.state.cash < ticket.cost;
  ...
  function renderSkills(game) {
    ...
    config.skillBranches.forEach((branch) => {
      const branchSection = document.createElement('section');
      branchSection.className = `incremental-skill-branch is-${branch.id}`;
      const branchHeading = document.createElement('div');
      branchHeading.className = 'incremental-skill-branch-heading';
      ...
      branchHeading.append(branchTitle, branchDescription);
      const branchNodes = document.createElement('div');
      branchNodes.className = 'incremental-skill-branch-nodes';

      config.skills.filter((skill) => skill.branchId === branch.id).forEach((skill) => {
        const rank = game.getSkillRank(skill.id);
        const unlock = game.getSkillUnlockStatus(skill.id);
        const card = document.createElement('article');
        card.className = 'incremental-skill-card';
        card.dataset.tier = String(skill.tier);
        card.classList.toggle('is-locked', !unlock.unlocked);
        const tier = document.createElement('span');
        tier.className = 'incremental-skill-tier';
        tier.textContent = `Tier ${formatNumber(skill.tier)}`;
        const header = document.createElement('div');
        ...
        button.textContent = !unlock.unlocked
          ? 'Prerequisite Required'
          : rank >= skill.maxRank
            ? 'Max Rank'
            : 'Spend 1 Point';
        button.disabled = !unlock.unlocked || rank >= skill.maxRank || available < 1;
        card.append(tier, header, description, effect);
        if (skill.prerequisites.length) {
          const requirements = document.createElement('div');
          requirements.className = 'incremental-skill-prerequisites';
          unlock.prerequisites.forEach((requirement) => {
            const note = document.createElement('span');
            note.classList.toggle('is-met', requirement.met);
            note.textContent = `${requirement.name} rank ${formatNumber(requirement.currentRank)} / ${formatNumber(requirement.rank)}`;
            requirements.appendChild(note);
          });
          card.appendChild(requirements);
        }
        card.appendChild(button);
        branchNodes.appendChild(card);
      });
      branchSection.append(branchHeading, branchNodes);
      nodes.skills_grid.appendChild(branchSection);
    });
  }
  ...
  function makeGeneratorCard(game, generator, automation) {
    ...
    const effectivePower = production?.effectivePower || 0;
    const card = document.createElement('article');
    ...
    total.textContent = `${formatNumber(effectivePower)} power/sec · ${config.minesById[production?.mineId]?.name || 'unassigned'}`;
  ...
  function renderCompany(game) {
    ...
    nodes.company_intro.textContent = acquired
      ? `${config.competition.rival.name} has been integrated into your operation. Personal and company worksites remain independently assigned.`
      : 'Each automation type works its own assigned deposit. Workers provide modest XP, while personal mining still offers critical hits and rare finds.';
  ...
  function showAutomation(result) {
    result.sites.forEach((site) => {
      if (!(site.depositsBroken > 0)) return;
      const rewards = Object.entries(site.resources)
        .map(([resourceId, quantity]) => `+${formatNumber(quantity)} ${config.resourcesById[resourceId]?.name || resourceId}`)
        .join(', ');
      automationSummaries.set(
        site.generatorId,
        `${formatNumber(site.depositsBroken)} deposit${site.depositsBroken === 1 ? '' : 's'} · ${rewards}`,
      );
    });
    if (!(result.depositsBroken > 0)) return;
    const now = performance.now();
    if (now - lastAutomationFeedbackAt < 750) return;
    lastAutomationFeedbackAt = now;
  ...
  function showOfflineProgress(result) {
    ...
    nodes.offline_production.textContent = `${formatNumber(result.depositsBroken)} deposit${result.depositsBroken === 1 ? '' : 's'} · ${formatNumber(result.damage)} mining damage${result.xp > 0 ? ` · ${formatNumber(result.xp)} XP` : ''}`;
    ...
    if (produced.length) {
      ...
      empty.textContent = 'Your assigned worksites weakened their deposits but did not finish one yet.';
      nodes.offline_resources.appendChild(empty);
    }
    nodes.offline_value.textContent = formatCurrency(result.estimatedValue);
    const notes = ['Offline automation preserves each generator type\'s mine assignment and separate deposit. Timed personal mining events do not boost offline rewards.'];
    if (result.capped) {
...
async function bootstrap() {
  ...
  game.subscribe((event) => {
    if (event.type === 'mine') {
      ui.showImpact(event.detail);
      if (event.detail.levelsGained > 0) ui.render(game);
      else ui.renderLive(game);
    }
  ...
  tabDefinitions.forEach(([tab, view]) => tab.addEventListener('click', () => {
    ui.render(game);
    ui.setView(view);
  }));
  ...
  ui.nodes.open_skills.addEventListener('click', () => { ui.render(game); ui.setView('skills'); });
  ...
  ui.nodes.skills_grid.addEventListener('click', (event) => {
    ...
    ui.nodes.skill_status.textContent = result.ok
      ...
      : result.reason === 'missing-prerequisite'
        ? `Requires ${result.unmet.name} rank ${formatNumber(result.unmet.rank)} first.`
  ...
  ui.nodes.store_categories.addEventListener('click', (event) => {
    ...
    if (equipmentButton) {
      ...
      ui.nodes.store_status.textContent = result.ok
        ...
        : result.reason === 'level-required'
          ? `${item.name} unlocks at character level ${formatNumber(result.requiredLevel)}.`
    ...
    if (buyTicketButton) {
      ...
      ui.nodes.store_status.textContent = result.ok
        ...
        : result.reason === 'level-required'
          ? `${ticket.name} unlocks at character level ${formatNumber(result.requiredLevel)}.`
  ...
  ui.nodes.crew_grid.addEventListener('change', (event) => {
    const select = event.target.closest('select[data-assign-generator-id]');
    if (!select) return;
    const result = game.assignGeneratorToMine(select.dataset.assignGeneratorId, select.value);
    const generator = database.config.generatorsById[select.dataset.assignGeneratorId];
    const mine = database.config.minesById[select.value];
    ui.nodes.crew_status.textContent = result.ok
      ? `${generator.name} assigned to ${mine.name}. Its separate worksite has opened a new deposit.`
      : 'That automation assignment could not be completed.';
    ui.renderLive(game);
  });
  ...
  function loop(now) {
    ...
    simulationAccumulator = Math.min(
      1,
      simulationAccumulator + Math.max(0, (now - lastFrame) / 1000),
    );
    let result = null;
    if (simulationAccumulator >= simulationStepSeconds) {
      result = game.update(simulationAccumulator);
      simulationAccumulator = 0;
    }
    if ((result?.automation?.damage > 0
      || result?.automation?.xp > 0
      || game.state.activeMiningEvent)
      && now - lastLiveRender >= 250) {
      ui.renderTick(game);
      lastLiveRender = now;
    }
    lastFrame = now;
