...
function validAutomationDepositMap(value) {
  if (!plainObject(value)) return false;
  return Object.entries(value).every(([generatorId, deposit]) => (
    validId(generatorId)
    && plainObject(deposit)
    && Object.keys(deposit).every((field) => ['id', 'hp', 'maxHp'].includes(field))
    && validId(deposit.id)
    && Number.isFinite(deposit.maxHp)
    && deposit.maxHp > 0
...
function generatorAssignmentMap(config) {
  return Object.fromEntries((config.generators || []).map((generator) => [
    generator.id,
    config.start.mineId,
  ]));
}

function automationDepositMap(config) {
  const deposit = config.depositsById[config.start.depositId];
  return Object.fromEntries((config.generators || []).map((generator) => [generator.id, {
    id: deposit.id,
    hp: deposit.maxHp,
    maxHp: deposit.maxHp,
  }]));
...
export function createInitialIncrementalSnapshot(config, options = {}) {
  ...
  return {
    ...
    generatorAssignments: generatorAssignmentMap(config),
    automationDeposits: automationDepositMap(config),
...
export function migrateIncrementalSnapshot(snapshot) {
  ...
  if (migrated.saveVersion === 7) {
    migrated.generatorAssignments = plainObject(migrated.generatorAssignments)
      ? migrated.generatorAssignments
      : {};
    migrated.automationDeposits = plainObject(migrated.automationDeposits)
      ? migrated.automationDeposits
      : {};
    const statistics = plainObject(migrated.statistics) ? migrated.statistics : {};
    statistics.totalAutomatedXp = nonnegativeFinite(statistics.totalAutomatedXp)
      ? statistics.totalAutomatedXp
      : 0;
    migrated.statistics = statistics;
...
export function validateIncrementalSnapshot(snapshot) {
  ...
  if (!plainObject(snapshot.generatorAssignments)
    || !Object.entries(snapshot.generatorAssignments).every(([generatorId, mineId]) => (
      validId(generatorId) && validId(mineId)
    ))) return false;
  if (!validAutomationDepositMap(snapshot.automationDeposits)) return false;
