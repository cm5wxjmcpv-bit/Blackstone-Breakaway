...
test('the standalone manifest accepts only the incremental runtime', () => {
  ...
  assert.throws(
    () => normalizeGameManifest({ id: 'bad-game', gameType: 'adventure' }, 'bad-game'),
    /Unsupported game type/,
  );
  assert.throws(
    () => normalizeGameManifest({ id: 'bad-game' }, 'bad-game'),
    /Unsupported game type/,
  );
});
...
test('mine reward multipliers make deeper configured mines pay more per matching deposit', () => {
  const richerConfig = config((raw) => {
    raw.mines[0].rewardMultiplier = 1.5;
  });
  const { game } = createGame(richerConfig);
  game.start();
  ...
  assert.equal(broken.mineRewardMultiplier, 1.5);
  assert.equal(broken.quantity, 3);
  assert.equal(game.state.materials.stone, 3);
});
...
test('skill branches enforce prerequisite ranks before higher tree nodes can advance', () => {
  const treeConfig = config((raw) => {
    raw.skillBranches = [
      { id: 'miner', name: 'Miner' },
    ...
    raw.skills.forEach((skill) => {
      skill.branchId = skill.id === 'ore-yield'
        ? 'prospector'
        : skill.id === 'automation-bonus'
          ? 'business'
          : 'miner';
      skill.tier = 1;
    });
    raw.skills.find((skill) => skill.id === 'critical-chance').tier = 2;
    raw.skills.find((skill) => skill.id === 'critical-chance').prerequisites = [
      { skillId: 'mining-power', rank: 2 },
    ];
  });
  const { game } = createGame(treeConfig);
  game.start();
  game.state.character.skillPoints = 4;

  const locked = game.allocateSkill('critical-chance');
  assert.equal(locked.reason, 'missing-prerequisite');
  assert.equal(locked.unmet.skillId, 'mining-power');
  assert.equal(locked.unmet.rank, 2);
  assert.equal(game.allocateSkill('mining-power').ok, true);
  assert.equal(game.allocateSkill('critical-chance').reason, 'missing-prerequisite');
  assert.equal(game.allocateSkill('mining-power').ok, true);
  assert.equal(game.getSkillUnlockStatus('critical-chance').unlocked, true);
  assert.equal(game.allocateSkill('critical-chance').ok, true);

  const malformed = rawConfig();
  malformed.skillBranches = [{ id: 'miner', name: 'Miner' }];
  malformed.skills.forEach((skill) => { skill.branchId = 'miner'; skill.tier = 1; });
  malformed.skills[1].tier = 2;
  malformed.skills[1].prerequisites = [{ skillId: 'missing-skill', rank: 1 }];
  assert.throws(
    () => normalizeIncrementalConfig(malformed, { gameId: 'miner-incremental' }),
...
test('store equipment is level-gated but does not require purchasing earlier gear', () => {
  const levelConfig = config((raw) => {
    const iron = raw.equipment.items.find((item) => item.id === 'iron-pickaxe');
    const steel = raw.equipment.items.find((item) => item.id === 'steel-pickaxe');
    delete iron.requiresItemId;
    delete steel.requiresItemId;
    iron.requiredLevel = 2;
    steel.requiredLevel = 5;
  });
  const { game } = createGame(levelConfig);
  game.start();
  game.state.cash = 100;

  const locked = game.purchaseEquipment('steel-pickaxe');
  assert.equal(locked.reason, 'level-required');
  assert.equal(locked.requiredLevel, 5);
  game.state.character.level = 5;
  const purchased = game.purchaseEquipment('steel-pickaxe');
  assert.equal(purchased.ok, true);
  assert.equal(game.state.ownedEquipment.includes('iron-pickaxe'), false);
  assert.equal(game.state.equipment.tool, 'steel-pickaxe');
});
...
test('lottery ticket tiers remain visible in data but purchases require their configured player level', () => {
  const tierConfig = config((raw) => {
    raw.lottery.scratchTickets[0].requiredLevel = 4;
  });
  const { game } = createGame(tierConfig);
  game.start();
  game.state.cash = 100;
  const locked = game.buyScratchTicket('test-scratch');
  assert.equal(locked.reason, 'level-required');
  assert.equal(locked.requiredLevel, 4);
  assert.equal(game.state.cash, 100);
  game.state.character.level = 4;
  assert.equal(game.buyScratchTicket('test-scratch').ok, true);
  assert.equal(game.state.cash, 90);
});
...
test('business upgrades and automation use a separate deposit while manual mining remains independent', () => {
  const { game } = createGame();
  game.start();
  game.state.storyStage = 'independent';
  game.state.employment.active = false;
  ...
  assert.equal(game.state.currentDeposit.hp, 4);
  assert.equal(game.state.automationDeposits['hired-miner'].hp, 2);
  assert.equal(game.state.character.xp, 0);
  assert.equal(game.state.materials.stone, 0);
  ...
  assert.deepEqual(game.state.currentDeposit, { id: 'stone-face', hp: 4, maxHp: 4 });
  assert.deepEqual(game.state.automationDeposits['hired-miner'], { id: 'stone-face', hp: 4, maxHp: 4 });
...
test('automation types can be reassigned to unlocked mines without touching the player deposit', () => {
  const progression = config(addMineProgressionContent);
  ...
  game.state.cash = 1_000;
  game.createCompany('Assignment Mining');
  game.purchaseGenerator('hired-miner');
  assert.equal(game.assignGeneratorToMine('hired-miner', 'iron-mine').reason, 'locked-mine');

  game.state.unlockedMines.push('iron-mine');
  game.random = () => 0.99;
  const assigned = game.assignGeneratorToMine('hired-miner', 'iron-mine');
  assert.equal(assigned.ok, true);
  assert.equal(game.getGeneratorAssignment('hired-miner'), 'iron-mine');
  assert.equal(game.state.automationDeposits['hired-miner'].id, 'iron-vein');
  const personalBefore = structuredClone(game.state.currentDeposit);
  const tick = game.update(8);
  assert.equal(tick.automation.sites[0].mineId, 'iron-mine');
  assert.equal(tick.automation.sites[0].depositsBroken, 1);
  assert.deepEqual(game.state.currentDeposit, personalBefore);
  assert.equal(game.state.mineProgress['iron-mine'].depositsBroken, 1);
  assert.equal(game.state.materials.iron, 3);
});

test('workers provide modest XP and a purchased training program increases automated XP income', () => {
  const xpConfig = config((raw) => {
    raw.balance.workerXpPerSecond = 0.1;
    raw.businessUpgrades.push({
      id: 'training-program',
  ...
  const { game } = createGame(xpConfig);
  game.start();
  game.state.storyStage = 'independent';
  game.state.employment.active = false;
  game.state.character.level = 2;
  game.state.cash = 1_000;
  game.createCompany('Learning Mining');
  game.purchaseGenerator('hired-miner');

  assert.equal(game.getAutomationXpPerSecond(), 0.1);
  const workerOnly = game.update(10).automation;
  assert.equal(workerOnly.xp, 1);
  assert.equal(game.state.character.xp, 1);
  assert.equal(game.purchaseBusinessUpgrade('training-program').ok, true);
  assert.equal(game.getAutomationXpPerSecond(), 0.6);
  const trained = game.update(2).automation;
  assert.ok(Math.abs(trained.xp - 1.2) < 1e-9);
  assert.ok(Math.abs(game.state.character.xp - 2.2) < 1e-9);
  assert.ok(Math.abs(game.state.statistics.totalAutomatedXp - 2.2) < 1e-9);
});
...
test('content changes reconcile a saved deposit that no longer belongs to its mine without resetting progress', () => {
  const progression = config(addMineProgressionContent);
  const snapshot = createInitialIncrementalSnapshot(progression, {
    now: 1000,
    gameVersion: '0.7.0',
  ...
  snapshot.cash = 321;
  snapshot.currentDeposit = { id: 'iron-vein', hp: 4, maxHp: 8 };
  const { game } = createGame(progression, {
    saveAdapter: { load: () => snapshot, save: () => true },
  });
  const started = game.start();
...
test('incremental saves round trip, migrate v1-v7 saves, and reject malformed or future data', (t) => {
  const originalWindow = globalThis.window;
  t.after(() => { globalThis.window = originalWindow; });
  globalThis.window = { location: { href: 'http://localhost/' } };
  const local = storage();
  const snapshot = createInitialIncrementalSnapshot(config(), { now: 1234, gameVersion: '0.8.0' });

  assert.equal(saveIncrementalGame(snapshot, 1, { storage: local, now: 1234 }), true);
  assert.ok(local.values.has('blackstone_breakaway_save_miner-incremental_slot_1'));
  ...
  const malformedAssignment = JSON.parse(JSON.stringify(snapshot));
  malformedAssignment.generatorAssignments['hired-miner'] = 'not a mine';
  assert.equal(validateIncrementalSnapshot(malformedAssignment), false);
  const malformedAutomationDeposit = JSON.parse(JSON.stringify(snapshot));
  malformedAutomationDeposit.automationDeposits['hired-miner'].hp = 99;
  assert.equal(validateIncrementalSnapshot(malformedAutomationDeposit), false);
  ...
  local.setItem('blackstone_breakaway_save_miner-incremental_slot_1', JSON.stringify({
  ...
  delete legacy.generatorAssignments;
  delete legacy.automationDeposits;
  delete legacy.statistics.totalAutomatedXp;
  ...
  local.setItem('blackstone_breakaway_save_miner-incremental_slot_1', JSON.stringify(legacyEnvelope));
  const migratedLegacy = loadIncrementalGame(1, { storage: local });
  assert.equal(migratedLegacy.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  assert.deepEqual(migratedLegacy.generatorAssignments, {});
  assert.deepEqual(migratedLegacy.automationDeposits, {});
  assert.equal(migratedLegacy.statistics.totalAutomatedXp, 0);
  ...
  assert.equal(migratedMilestoneTwo.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  assert.equal(reconciledGame.state.gameVersion, '0.8.0');
  ...
  assert.equal(migratedMilestoneThree.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  assert.equal(migratedMilestoneFour.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  assert.equal(migratedMilestoneFive.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  assert.equal(migratedMilestoneSix.saveVersion, INCREMENTAL_SAVE_VERSION);
  ...
  const milestoneSeven = JSON.parse(JSON.stringify(snapshot));
  milestoneSeven.saveVersion = 7;
  delete milestoneSeven.generatorAssignments;
  delete milestoneSeven.automationDeposits;
  delete milestoneSeven.statistics.totalAutomatedXp;
  const migratedMilestoneSeven = migrateIncrementalSnapshot(milestoneSeven);
  assert.equal(migratedMilestoneSeven.saveVersion, INCREMENTAL_SAVE_VERSION);
  assert.deepEqual(migratedMilestoneSeven.generatorAssignments, {});
  assert.deepEqual(migratedMilestoneSeven.automationDeposits, {});
  assert.equal(migratedMilestoneSeven.statistics.totalAutomatedXp, 0);
  ...
  assert.equal(migrateIncrementalSnapshot(versionZero).saveVersion, INCREMENTAL_SAVE_VERSION);
});
...
test('Milestone 4 saves reconcile new resources, skills, mines, and progress without resetting the player', () => {
  ...
  assert.equal(game.state.cash, 321);
  assert.equal(game.state.gameVersion, '0.8.0');
